...
function CountProduct(props) {
    ...
    let [kafCalculate, setkafCalculate] = useState((getCookie("calculat-kaf") !== null) ? getCookie("calculat-kaf") : null)
    let [divarCalculate, setdivarCalculate] = useState((getCookie("calculat-divar") !== null) ? getCookie("calculat-divar") : null)
    let [price, setPrice] = useState([])
    let [priceId, setPriceId] = useState([])
    let [priceMerchantId, setPriceMerchantId] = useState(null)
    ...
    useMemo(() => {
        ...
        if (products) {
            (products)?.map(function (product, key_) {
                ...
                priceId_[key_] = product.priceid; setPriceId(priceId_)
                ...
                product.extrafieldvalues &&
                    Object.entries(product.extrafieldvalues)?.map(([key, extra]) => {
                        size_[key_] = (product.ratio) ? Number(product.ratio) : 1;
                         setSize(size_)
                        if (extra.field === "متراژ در کارتن") {
                            // if(!divarCalculate && !kafCalculate){
                            //     return(setCount(count => [...count, 1]),count_.push(1))
                            // }
                            return (
                                // (!divarCalculate && !kafCalculate) ? (setCount(count => [...count, 1]),count_[key_] = 1) : '',
                                // setSize(size => [...size, Number((extra.value).replace('/','.'))]),size_.push(Number((extra.value).replace('/','.'))),
                                // setPrice(price => [...price, product.price]),price_.push(product.price),
                                size_[key_] = (product.ratio) ? Number(product.ratio) : Number((extra.value).replace('/', '.')), setSize(size_),
                                price_[key_] = product.price, setPrice(price_),
                                priceId_[key_] = product.priceid, setPriceId(priceId_),
                                discount_[key_] = product.discount, setdiscount(discount_),
                        ...
                        if ((extra.field === "قطعات" || extra.field === "نام محصول") && extra.value) {
                            return (
                                <>
                                    {setproductname((extra.value) ? extra.value : product.productcode)}
                                </>
                            )
                        }
                        ...
                        if (extra.fieldid === 6 && (extra.value)?.includes('دیوار') && divarCalculate) {
                            return (
                                // count_.push(Number(divarCalculate)),setCount(count_)
                                count_[key_] = Number(divarCalculate), setCount(count_)
                            )
                        } else if (extra.fieldid === 6 && (extra.value)?.includes('کف') && kafCalculate) {
                            return (
                                // count_.push(Number(kafCalculate)),setCount(count_)
                                count_[key_] = Number(kafCalculate), setCount(count_)
                            )
                            ...
                            return (
                                // setCount(count => [...count, 1]),count_.push(1)
                                // count_.push(Number(1)),setCount(count_)
                                count_[key_] = Number(kafCalculate), setCount(count_)
                            )
                        }
        ...
        <>
            <form   method="post" onSubmit={handleSubmit(cartSubmit)} className="pcp-hidden ajaxCart">
                                        ...
                                        <>
                                            <li className="list-group-item metraj">
                                                <div className="row">
                                                    ...
                                                    {(product.avail_type !== 'عدد' && product.avail_type !== 'مترمربع') &&
                                                        <div className="col-6 text-center mtr-con">

                                                            {product.avail_type === 'کارتن' ?
                                                    ...
                                                    {/* <input type="hidden" name={`priceid[${key}]`} value={priceId[key]} /> */}
                ...
                <div className="col-lg-12 col-md-12 col-sm-12 pt-2">
                    ...
                    <div className="product-params default parent-btn-sell p-1">
...
export default withRouter(CountProduct);